...
class BurpExtender(IBurpExtender, IContextMenuFactory):
    def registerExtenderCallbacks(self, callbacks):
        ...
        # we set up our extention 
        callbacks.setExtensionName("BHP Bing")
        callbacks.registerContextMenuFactory(self)

        return 

    def createMenuItems(self, context_menue):
        self.context = context_menue
        menu_list = ArrayList()
        menu_list.add(JMenuItem("send to Bing", actionPerformed=self.bing_menu))
        return menu_list

    def bing_menu(self,event):
        # grab the details of what the user clicked on
    ...
    def bing_search(self, host):
        # first check if we have an IP or host name 
        try:
    ...
    def bing_query(self, bing_query_string):
        ...
        try:
            ...
            print('No resulsts from Bing: %s' % err)
        else:
            sites = list()
